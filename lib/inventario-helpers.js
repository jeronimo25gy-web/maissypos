...
export const calcularStockPorSku = async () => {
  ...
  const [{ data: conteos }, { data: productosAll }] = await Promise.all([
    supabase
      .from('conteo_fisico')
    ...
    supabase.from('productos').select('sku').eq('empresa_id', empresaId),
  ])
  ...
  ;(productosAll || []).forEach(p => {
    if (!(p.sku in stockPorSku)) stockPorSku[p.sku] = { cantidad: 0, fecha: '1900-01-01' }
  })
  ...
  if (fechaMinima) {
    const { data: liquidaciones } = await supabase
      .from('liquidaciones')
      .select('sku, devuelto, fecha')
      .eq('empresa_id', empresaId)
      .gt('devuelto', 0)
      .gte('fecha', fechaMinima)
    ;(liquidaciones || []).forEach(l => {
      const stockInfo = stockPorSku[l.sku]
      if (!stockInfo || l.fecha < stockInfo.fecha) return
