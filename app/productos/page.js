...
export default function Productos() {
  ...
  const utilidadResultante = (precio, costo) => {
    if (!precio || !costo) return null
  ...
  const agregarProducto = async () => {
    if (!nuevo.sku || !nuevo.nombre || !nuevo.precio_venta) { alert('SKU, nombre y precio son obligatorios'); return }
    setGuardando(true)
        ...
        {data.precio_venta && data.costo_compra && (
          <div className="bg-green-50 rounded-lg p-3 mt-2 flex justify-around">
            <div className="text-center">
              ...
              <p className="font-black text-green-600">${utilidadResultante(data.precio_venta, data.costo_compra)?.toLocaleString('es-CO')}</p>
            </div>
            <div className="text-center">
