  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <div className="bg-white shadow-sm px-6 py-4 flex justify-between items-center sticky top-0 z-10">
        <div>
          <h1 className="text-xl font-black text-teal-600">Inventario</h1>
      ...
      <div className="p-4 max-w-3xl mx-auto">
        <input type="text" placeholder="Buscar por nombre o SKU..." value={busqueda}
          ...
          className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 mb-3 focus:border-teal-500 focus:outline-none" />

        <button onClick={() => setSoloBajoMinimo(!soloBajoMinimo)}
          className={`text-xs font-bold px-3 py-2 rounded-lg mb-4 ${soloBajoMinimo ? 'bg-red-500 text-white' : 'bg-white text-gray-600 border border-gray-200'}`}>
          Solo bajo minimo
        </button>
        ...
        {cargando ? (
          ...
          <div className="bg-white rounded-xl shadow-sm divide-y divide-gray-100">
            {filasFiltradas.map(p => (
              <div key={p.id} className={`p-4 flex items-center justify-between ${p.bajoMinimo ? 'bg-red-50' : ''}`}>
                <div className="flex-1">
                  ...
                  {p.fechaConteo ? (
                    ...
                    <p className="text-xs text-orange-500 font-bold">Sin conteo registrado</p>
                  )}
                </div>
                <div className="flex gap-4 items-center">
                  ...
                  <div className="text-center w-16">
                    ...
                    <p className={`text-xl font-black ${p.bajoMinimo ? 'text-red-600' : 'text-gray-800'}`}>
